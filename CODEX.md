...
This file provides default Codex-style instructions for working in this
repository.

The repository contains a Python implementation of a strict Semantic Versioning
2.0.0 validator, along with:
...
Any change to parsing or validation must remain compliant with the Semantic
...
Do not change output keys casually. The project is currently at `0.5.0`, so
pre-`1.0.0` changes are still possible, but they should be intentional and
documented clearly. The goal for `1.0.0` is to stabilize response shape, CLI
behavior, and API behavior.
...
- avoid introducing response formats that diverge from the validator without a
  strong reason
...
When modifying validator behavior, use examples like these to reason-check
changes.
...
1. understand whether it affects core validation, CLI behavior, API behavior, or
   packaging
...
In this repository, correctness is more important than cleverness, and
